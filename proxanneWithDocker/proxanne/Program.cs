...
public class proxanne
{
                ...
                using (remoteServer)
                {
                    using (writer = new StreamWriter("TCPProxy.log", append: true))
                    {
                        try
                        {
                            ...
                            {
                                remoteEndPoint = new IPEndPoint(Dns.GetHostAddresses("host.docker.internal")[0], 8000);
                                writer.WriteLine("Google catched : forward to localhost:8000");
                            }
                            else
                            {
                                remoteEndPoint = new IPEndPoint(Dns.GetHostAddresses("host.docker.internal")[0], 7000);
                            }
    ...
    public static async Task Main(String[] args)
    {
        ...
        await Proxy.start(new IPEndPoint(Dns.GetHostAddresses("host.docker.internal")[0], 9000));
    }
}